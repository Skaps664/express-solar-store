...
export default function BlogClient({ initialBlog, initialRelatedBlogs }: { initialBlog: Blog | null, initialRelatedBlogs: Blog[] }) {
  ...
  useEffect(() => {
    // Only fetch if we weren't given server data
    if (blog) {
      setLoading(false)
      return
    }
    ...
    const fetchBlog = async () => {
      try {
        ...
        // fallback: do nothing (server should provide data). Keep minimal client fetch if needed.
      } catch (error) {
        console.error('Error fetching blog (client fallback):', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <article className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      {/* The rest (featured products/brands/related blogs) is unchanged and rendered below */}
