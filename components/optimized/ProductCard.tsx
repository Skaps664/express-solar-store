...
const ProductCard = memo(({ product, priority = false }: ProductCardProps) => {
  ...
  const imageUrl = useMemo(() => {
    ...
    // R2 images are already pre-optimized with sharp during upload — serve as-is
    const r2Domain = process.env.NEXT_PUBLIC_R2_PUBLIC_URL || '';
    if (r2Domain && url.includes(r2Domain.replace('https://', '').replace('http://', ''))) {
      return url;
    }
    // Add Cloudinary optimizations if using Cloudinary (old images)
