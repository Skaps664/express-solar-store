...
export default function MiniBottomSolarAd() {
  const [adData, setAdData] = useState<MiniBottomAdData>({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (data?.miniBottomAd) {
          setAdData(data.miniBottomAd);
        }
      } catch (error) {
        console.error("Error fetching mini bottom ad data:", error);
      }
    };
